...
_GLFWmonitor* createMonitor(DISPLAY_DEVICEW* adapter, DISPLAY_DEVICEW* display)
{
    ...
    if (display)
        ...
        name = _glfwCreateUTF8FromWideString(adapter->DeviceString);
    if (!name)
    {
...
{
    int adapterCount = 0, displayCount = 0, found = 0;
    ...
    *count = 0;
    ...
    if (displayCount)
        ...
        monitors = calloc(adapterCount, sizeof(_GLFWmonitor*));
    ...
    for (adapterIndex = 0;  ;  adapterIndex++)
    {
        ...
        if (displayCount)
        {
            for (displayIndex = 0;  ;  displayIndex++)
            {
                ...
                monitors[found++] = createMonitor(&adapter, &display);
            }
        }
        else
            monitors[found++] = createMonitor(&adapter, NULL);
    }
...
{
    if (wsclen(first->win32.displayName) && wsclen(second->win32.displayName))
